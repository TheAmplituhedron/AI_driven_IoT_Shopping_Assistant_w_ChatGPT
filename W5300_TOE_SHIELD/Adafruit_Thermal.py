...
class Adafruit_Thermal:
	...
	def __init__(self, bus=1, baudrate=9600, pins=None, **kwargs):
		...
		heatDots = kwargs.get('heatdots', self.defaultHeatDots)
		heatTime = kwargs.get('heattime', self.defaultHeatTime)
		heatInterval = kwargs.get('heatinterval', self.defaultHeatInterval)
		self.writeBytes(
		  27,           # Esc
		  55,           # 7 (print settings)
		  heatDots,     # Heat dots
	...
	def printBarcode(self, text, type):
		...
		upcCodes = { # UPC codes & values for firmwareVersion >= 264
			self.UPC_A   : 65,
			self.UPC_E   : 66,
			self.EAN13   : 67,
			self.EAN8    : 68,
		...
		# Print string: write length byte + string sans NUL
		n = len(text)
		if n > 255: n = 255
	...
	def sleepAfter(self, seconds):
		self.writeBytes(27, 56, min(seconds,0xFF))
	...
	def hasPaper(self):
		...
		stat = ord(self.uart.read(1)) & 0b00000100
